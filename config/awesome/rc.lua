...
vicious.register(netwidget, vicious.widgets.net, function(widget, args)
    ...
    --return '<span background="#C2C2A4" font="Inconsolata 11"> <span font ="Inconsolata 11" color="#ff8080">'..args["{"..interface.." down_kb}"]..'kbps'..'</span></span>' end, 10)
...
cpuicon.image = beautiful.cpuicon

--{{--| MEM widget |-----------------
memwidget = wibox.widget.textbox()

vicious.register(memwidget, vicious.widgets.mem, 'Mem:$2MB ', 20)
...
local mdir = { "/home/ross/mail/personal", "/home/ross/mail/uni"}

vicious.register(mailtext, vicious.widgets.mdir, ' $1 ', 15, mdir)
vicious.register(mailicon, vicious.widgets.mdir, function(widget, args)
  local newMail = args[1]
...
config_dir = ("/home/ross/.config/awesome/")

beautiful.init(awful.util.getdir("config") .. "/themes/ross/theme.lua")
--beautiful.init(awful.util.get_themes_dir() .. "ross/theme.lua")
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey },            "r",     function ()
      awful.util.spawn("dmenu_run -i -p 'Run command:' -l 30") 
    end ),

    awful.key({ modkey },            "w",     function ()
      awful.util.spawn("dswitcher") 
    end ),
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "0", awful.titlebar.toggle),
...
awful.rules.rules = {
    ...
    { rule_any = {
        instance = {
          "wine",  -- Firefox addon DownThemAll.
          "wow.exe",  -- Includes session name in class.
        },
      }, properties = { floating = true }},
...
keychains.add ({modkey}, "e", "MPD", "", {
  n = {
    func = function() awful.util.spawn("bash -c \"MPD_HOST=/tmp/mpd.sock mpc next\"",false) end,
    info = "next"},
    b = {
      func = function() awful.util.spawn("bash -c \"MPD_HOST=/tmp/mpd.sock mpc prev\"",false) end,
      info = "previous"},
      p = {
        func = function() awful.util.spawn("bash -c \"MPD_HOST=/tmp/mpd.sock mpc toggle\"",false) end,
        info = "Play/Pause"},
        c = {
          func = function() awful.util.spawn("bash -c \"MPD_HOST=/tmp/mpd.sock mpc -f '%artist% - %title%' |head -n 1 - | perl -pe 'chomp' | DISPLAY=:0.0 xsel -i\"",false) end,
          info = "Copy np"}
        })
        keychains.start(5)
--awful.util.spawn_with_shell("~/bilder/wallpaper/wallpaper.sh ~/bilder/wallpaper/cur")
